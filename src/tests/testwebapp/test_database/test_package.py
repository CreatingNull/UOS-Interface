...
class TestInterface:
    ...
    def test_add_user_privilege(
        ...
        newer_privilege = (
            db_session.query(Privilege)
            .filter(Privilege.name == "NewerPrivilege")
            .first()
        )
    ...
    @staticmethod
    def test_init_privilege(db_session):
        """
        ...
        assert (
            db_session.query(Privilege).filter(Privilege.name == "NewPrivilege").first()
        )
        # Check duplication of privilege raises error correctly
        with pytest.raises(UOSDatabaseError):
            init_privilege(
                db_session, test_privilege["name"], test_privilege["description"]
            )
